...
项目首先服务于 Agent 评测研发实践、受控外部工具集成评测和 Chora 完成态任务评测。
...
| P4.5 | 真实工具集成评测 | D1 已有探索运行；有效基线仍待关联案例分组、人工参考校准和实际执行条件冻结，后续扩展 D2/D3 消费场景及 CLI/skill 对照。 |
...
**状态：** D1 已有探索运行，但工具执行无效或条件不可核验的历史运行不进入确认性比较。
先冻结关联案例分组、人工参考与实际执行条件，重建有效基线，再做单变量干预并在未参与调优的
独立验证集上比较。D1 是首个能力切片，
不代表完整的外部工具 Agent 友好度评测。
正向提升尚未证明。

### 目标与执行顺序

1. 选择一个真实工具使用任务，保存案例、工具响应、必要输入及人工参考判断，
   形成可重放输入。将关联案例分在同一组，再划分开发集和独立验证集。
2. 固定被测 Agent、模型、Prompt、工具访问与预算。评估任务完成度和输出质量，
