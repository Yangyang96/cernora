...
当前主线是 **中立接入与教程 → P4.5 有证据支持的改进验证 → Chora 最小接入**。
当前评测切片检查 Agent 能否正确消费工具证据、输出符合要求的结构化答案。这是验证有效
干预的起点，尚不能证明完整任务质量。通用能力由已证实的需求驱动，不以补齐其他平台
全部功能为完成条件。

状态对齐日期：2026-09-16。公共交付就绪、开发实验完成和独立收益证明分别记录。

| 顺序 | 里程碑 | 当前安排 |
| --- | --- | --- |
| P1–P3 | 指标、Profile Authoring、Reference Workflow | 保留已交付能力与兼容边界。 |
| P4 | 批量实验、比较与证据闭环 | 72 Trials 已完成，负向结果与离线重建已封存；公开发布单独跟踪。 |
| P4.5 | 证据消费与改进验证 | 开发评测已跑通；独立参考校准、确认性基线、单变量干预及未参与调优的验证仍未完成。 |
...
| 原 P7 / P8 | 通用 LLM Judge / 稳定化 | 条件储备：参考校准后仍有明确评分缺口时才引入 Judge。 |
...
**状态：** 进行中。中立最小 Adapter 示例和首次接入教程已经实现，并通过独立安装环境
验证。基于冻结工具证据的结构化答案开发评测已跑通。独立人工参考校准、确认性基线和
干预验证仍未完成。历史无效运行继续排除，保留原证据，不改写历史评分。

### 当前评测范围

当前任务检查查询对象身份、与父对象的关系、状态字段、目标对象、完整性、缺失说明，
以及对实际工具结果的引用。Profile 将结构化声明与源绑定参考逐项比较，区分有有效证据
但回答错误，以及答案或证据不可用。工具调用成功或导出格式正确都不能单独证明任务成功。

这一切片可以暴露字段理解、事实依据和输出契约问题，尚未证明领域判断、根因准确性、
多步规划或完整用户任务效用。格式符合率提高只能按该指标报告；更广泛的改善声明需要
独立任务结果支持。
...
| 步骤 | 工作 | 当前状态 |
| --- | --- | --- |
| 1 | 中立最小 Adapter：从合成运行生成导出，覆盖成功、错误答案、缺证和字段错误 | 已实现并验证。 |
| 2 | 在独立安装环境完成首次接入，区分 inspect 与完整 Profile 评测 | 教程与自动化安装验收已完成。 |
| 3 | 公共交付与状态对齐 | 通用接入成果已交付；持续对齐文档，不公开私有实验。 |
| 4 | 重建分组数据、校准参考、固定实际运行条件 | 已有开发证据；独立校准和确认性基线未完成。 |
| 5 | 单变量干预与未参与调优案例上的独立验证 | 待完成；如实报告收益、回归、成本、不确定性及负面结果。 |
| 6 | 提炼已证实的通用缺口并准备版本 | 由证据触发；完成合成覆盖、兼容性、构建及 preflight，发布另行执行。 |

`inspect` 检查用于检查投影的导出并生成观察结果，不导入 EvidenceBundle，也不执行
Profile 评分。完整验收需要导入、Profile
评测和严格重载，并覆盖成功、失败与缺证路径。参见
[首次接入教程](docs/public/first-integration.zh-CN.md)。

### 实验要求

1. 先按关联关系分组，再划分开发与验证数据。标识不同不自动证明独立；重复运行不能当作
   独立任务样本。
2. 源事实核对与独立参考校准分别记录。确认性评测前冻结任务、预期事实、评分口径、排除
   规则与缺证处理。
3. 固定实际 Agent、模型、提示、工具访问、输入摘要、预算和重试规则，保留失败及无定论
   运行。脱敏衍生输入具有独立身份和参考，不能把其结果直接转移到原始输入。
4. 根据开发失败选择一个干预，保持评分及其他条件不变；新案例用于独立验证，不用于调优提示。
5. 同时报告任务结果、回归、重复运行差异和成本，计入重复工具调用，明确不可用的测量值。
   token 下降不能单独证明成本收益。

开发观察可以提出输出格式干预，但尚不能证明其有效。更复杂任务、其他工具接口和多轮
研究属于条件扩展，不作为完成当前有界实验的前置条件。

### Judge 边界

当前字段检查采用确定性、源绑定评分，不需要 LLM Judge。只有在制定量表并完成人工
校准后仍有具体定性评分需求，才引入范围受限的外部 Judge。保留其输入、版本和证据，
不可用的判断保持无定论。Core 继续离线评测完成态导出，不取得模型凭据或运行职责。
通用 Judge SDK 仍是条件储备。

### 完成信号与停止条件

至少一次干预在独立验证案例上、满足预声明的结果、回归和成本约束时产生有证据支持的
实际改善。只声明确实测量的范围，输出契约符合率不能代替业务效用。退步、无定论或证据
不足时，改进里程碑保持未完成。Preview 可以按已说明的契约交付，但不能因此宣称干预有效
或 Agent 能力稳定。复用现有 Profile、Evidence Adapter 与报告，只补证据确认的通用能力。
...
P7 是通用 Judge 接口，P8 是其稳定化。经过校准的定性任务确有需求时才考虑辅助 Judge；
当前确定性任务不需要它。
